...
public class TestShutdown {
    ...
    public static void main(String[] args_) throws CacheException {
        for (int i = 0; i < args_.length; i++) {
            System.out.println(">>args[" + i + "]:" + args_[i]);
        }

/*        if (args_.length != 1) {
            System.out.println("Usage: <JVM> TestShutdown <config> log4j.properties <config>");
       ...
       // PropertyConfigurator.configure(args_[0]);
        ...
        TestShutdown test = new TestShutdown();
        test.doIt();
    }
